...
class D3Graph {
  constructor(element) {
    ...
    // vis.color = '#5fb8c2'        // teal
    // vis.color = '#E49273'        // rose gold
  ...
  update(graphStructure) {
    ...
    let graphLinks = vis.svg
      ...
      .join(
        ...
        (update) => update.attr("fill", vis.color2),
        (exit) => exit.remove()
      );

    // append title to all links
    graphLinks
      .append("title")
      .text((graphLink, i) => `Salutations, I'm edge ${i}`);

    // data join the nodes
    let graphNodes = vis.svg
      .append("g")
      .attr("stroke", "white")
      .attr("stroke-width", 1)
      .selectAll("circle")
    ...
    // append title to all nodes
    graphNodes.append("title").text((graphNode, i) => `Hii I'm node ${i}`);
