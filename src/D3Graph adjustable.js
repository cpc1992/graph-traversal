...
class D3Graph {
  constructor(element) {
    ...
    link.append("title").text((d, i) => `Salutations, I'm edge ${i}`);
    ...
    let node = svg
      ...
      .data(nodes)
      .join(
        // join merges the update and enter selections
    ...
    node.append("title").text((p, i) => `Hii I'm node ${i}`);
  ...
  update() {
    ...
    let selection = vis.svg.selectAll("rect").data(vis.data, (p) => p.name);
